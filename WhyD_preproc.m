...
%gm_mask_new = gm_mask.img; gm_mask_new(gm_mask.img>0) = 1;
%csf_mask_new = csf_mask.img; csf_mask_new(csf_mask.img>0) = 1;
wm_template = flair_ref.img .* wm_mask_new;
%gm_template = flair_ref.img .* gm_mask_new;
%csf_template = flair_ref.img .* csf_mask_new;
%
%nii.img = wm_template; save_nii(nii, sprintf('%s/WM_strip_%s.nii',names.directory_path,names.folder_id));
%nii.img = gm_template; save_nii(nii, sprintf('%s/GM_strip_%s.nii',names.directory_path,names.folder_id));
%nii.img = csf_template; save_nii(nii, sprintf('%s/CSF_strip_%s.nii',names.directory_path,names.folder_id));
