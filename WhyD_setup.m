...
% options include 'yes', 'no' (case insensitive) ; default is 'yes'
% do_visualize : creating and displaying the heatmap or not
% options include 'yes', 'no' (case insensitive) ; default is 'yes'
...
for n = 1:num
    ...
    if ~exist(names.directory_path, 'dir'), mkdir(names.directory_path); end
...
if strcmpi(do_train,'yes') || ~check_training(training_path)
    ...
    docker_run('python W2MHS_training.py False', training_path);
